...
import com.sist.mapper.QnAmapper;
...
public class QnADAO {
	   ...
	   @Autowired
	   private QnAmapper mapper;// 메모리 할당시 추가 
	   ...
	   */
	   // 목록 출력
	   public List<QnAVO> replyBoardListData(Map map)
	   {
	   ...
	   public void replyBoardReplyInsert(int pno,QnAVO vo)
	   {
		   // 1. conn.setAutoCommit(false) => Spring처리 ==> @Around
		   // 2. 사용자 호출 
		   // 2-1. 답변 대상 정보 읽기
		   QnAVO pvo=mapper.replyParentInfoData(pno);
		   // 2-2. group_step변경
	   ...
	   // 수정할 데이터 읽어 오기 
	   public QnAVO replyUpdateData(int no)
	   {
	   ...
	   public int replyDelete(int no,String pwd)
	   {
		   ...
		   if(db_pwd.equals(pwd))
		   {
			   result=1;
			   // root,depth를 읽어 온다 
			   QnAVO vo=mapper.replyboardGetDepth(no);
