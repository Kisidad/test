package com.sist.mapper;

import java.util.List;
import java.util.Map;

import org.apache.ibatis.annotations.Delete;
import org.apache.ibatis.annotations.Insert;
import org.apache.ibatis.annotations.Select;
import org.apache.ibatis.annotations.SelectKey;
import org.apache.ibatis.annotations.Update;


import com.sist.vo.QnAVO;


public interface QnAmapper {
	// 목록 출력 => 페이징 (인라인뷰) 
	   @Select("SELECT no,subject,name,dbday,hit,group_tab,num "
			  +"FROM (SELECT no,subject,name,TO_CHAR(regdate,'YYYY-MM-DD') as dbday,hit,group_tab,rownum as num "
			  +"FROM (SELECT no,subject,name,regdate,hit,group_tab "
			  +"FROM spring_replyboard1 ORDER BY group_id DESC , group_step ASC)) "
			  +"WHERE num BETWEEN #{start} AND #{end}")
	   public List<QnAVO> replyBoardListData(Map map); // 매개변수는 1개만 사용한다, 입력값이 한개이상 (Map,VO)
	   // 총페이지 
	   @Select("SELECT CEIL(COUNT(*)/10.0) FROM spring_replyboard1")
	   public int replyboardTotalPage();
	   // 추가 => no(primary key) => 마이바티스 지원 , 서브쿼리 , 시퀀스  
	   @SelectKey(keyProperty="no", resultType=int.class , before=true,
			     statement="SELECT NVL(MAX(no)+1,1) as no FROM spring_replyboard1")
	   // 자동 증가 번호를 만든다 
	   @Insert("INSERT INTO spring_replyboard1(no,name,subject,content,pwd,group_id) VALUES("
			  +"#{no},#{name},#{subject},#{content},#{pwd},"
			  +"(SELECT NVL(MAX(group_id)+1,1) FROM spring_replyboard1))")
	   public void replyBoardInsert(QnAVO vo);
	   // 내용보기 
	   @Update("UPDATE spring_replyboard1 SET "
			  +"hit=hit+1 "
			  +"WHERE no=#{no}")
	   // 함수를 이용하면 => as 반드시 사용한다 
	   public void hitIncrement(int no);
	   @Select("SELECT no,name,subject,content,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') as dbday,hit "
			  +"FROM spring_replyboard1 "
			  +"WHERE no=#{no}")
	   public QnAVO replyBoardDetailData(int no);
	   
	   // 답변 (SQL=>4개)
	   // 1. 답변 대상의 group_id,group_step,group_tab => group_id는 그대로 첨부 , group_step,group_tab은 +1
	   @Select("SELECT group_id,group_step,group_tab FROM spring_replyboard1 WHERE no=#{no}")
	   public QnAVO replyParentInfoData(int no);
	   /*                    group_id,group_step,group_tab  depth
	    *    AAAAAA            1          0         0         1  => 삭제여부 확인 (Update 관리자 삭제한 게시물)
	    *      =>BBBBB         1          1         1         1
	    *       =>CCCCC        1          2         2         1
	    *        =>DDDDD       1          3         3         0
	    *    FFFFFF            2          0         0
	    *      =>KKKKKK        2          1         1
	    *       => OOOOO       2          2         2
	    *                 같은 답변을 모아서 관리 
	    *                 
	    *    FFFFFF            2          0         0      2
	    *      =>PPPPP         2          1         1      0
	    *      =>KKKKKK        2          2         1      1
	    *       => OOOOO       2          3         2    2 0 0
	    *      
	    */
	   // 2. 첨부할때 group_step을 증가 (1) => 핵심 SQL
	   @Update("UPDATE spring_replyboard1 SET "
			  +"group_step=group_step+1 "
			  +"WHERE group_id=#{group_id} AND group_step>#{group_step}")
	   public void replyboardGroupStepIncrement(QnAVO vo);// vo에 없는 변수 => Map
	   // 3. 실제 데이터 추가 
	   @SelectKey(keyProperty="no", resultType=int.class , before=true,
			     statement="SELECT NVL(MAX(no)+1,1) as no FROM spring_replyboard1")
	   // 자동 증가 번호를 만든다 
	   @Insert("INSERT INTO spring_replyboard1(no,name,subject,content,pwd,group_id,group_step,"
	   		  +"group_tab,root,depth) VALUES("
			  +"#{no},#{name},#{subject},#{content},#{pwd},"
			  +"#{group_id},#{group_step},#{group_tab},#{root},0)")
	   public void replyboardReplyInsert(QnAVO vo);
	   // 4. depth 증가
	   @Update("UPDATE spring_replyboard1 SET "
			  +"depth=depth+1 "
			  +"WHERE no=#{no}")
	   public void replyboardDepthIncrement(int no);
	   // 1=>SELECT , 2=>UPDATE , 3.INSERT , 4.UPDATE  ==> 자동 저장 :AutoCommit
	   // 일괄 처리 => SQL문장 전체가 에러가 없는 경우 (COMMIT수행) ,  SQL문장중에 한개라도 에러가 있는 경우 ROLLBACK
	   
	   // 수정하기 
	   // 1. 비밀번호 검색 
	   @Select("SELECT pwd FROM spring_replyboard1 WHERE no=#{no}")
	   public String replyboardGetPassword(int no);
	   // 2. 실제 수정 
	   @Update("UPDATE spring_replyboard1 SET "
			  +"name=#{name},subject=#{subject},content=#{content} "
			  +"WHERE no=#{no}")
	   public int replyboardUpdate(QnAVO vo);
	   
	   // 삭제하기 => SQL (트랜잭션 적용)
	   // 1. 비밀번호 검색 => 재사용 (public String replyboardGetPassword(int no))
	   // 2. 비밀번호가 맞는 경우 
	   // 2-1. root,depth 읽기 
	   @Select("SELECT root,depth FROM spring_replyboard1 WHERE no=#{no}")
	   public QnAVO replyboardGetDepth(int no);
	   // 2-2. depth==0  ==> DELETE (삭제) => 비밀번호 (O) , depth=0
	   @Delete("DELETE FROM spring_replyboard1 WHERE no=#{no}")
	   public void replyDelete(int no);
	   // 2-3. depth!=0  ==> UPDATE (제목 : 관리자가 삭제한 게시물입니다)
	   @Update("UPDATE spring_replyboard1 SET "
			  +"subject=#{subject},content=#{content} "
			  +"WHERE no=#{no}")
	   public void replyboardDelUpdate(QnAVO vo);
	   // 2-4. depth 감소 => UPDATE
	   @Update("UPDATE spring_replyboard1 SET "
			  +"depth=depth-1 "
			  +"WHERE no=#{no}")
	   public void depthDecrement(int no);
	   
	   @Select("SELECT depth FROM spring_replyboard1 WHERE no=#{no}")
	   public int replyboardRootGetDepth(int no);
	   // 검색 => 동적 쿼리 
	   // 마이바티스 => 동적 쿼리 , 어노테이션 (JOIN) , PL/SQL ==> CURD (INSERT/UPDATE/DELETE/SELECT)
	   // @SELECT({"SELECT * FROM spring_replyboard1 WHERE "
	   //           +"<script><trim prefix="("
	   // 동적 쿼리 사용 => <script> => 태그이용한다
	   /*
	    *     N
	    *     WHERE name LIKE '%'||#{ss}||'%'
	    *     
	    *     S
	    *     WHERE subject LIKE '%'||#{ss}||'%'
	    *     
	    *     C
	    *     WHERE content LIKE '%'||#{ss}||'%'
	    *     
	    *     N / S
	    *     WHERE OR name LIKE '%'||#{ss}||'%' OR subject LIKE '%'||#{ss}||'%'
	    *     N / C
	    *     WHERE OR name LIKE '%'||#{ss}||'%' OR content LIKE '%'||#{ss}||'%'
	    *     N / S / C
	    *     WHERE name LIKE '%'||#{ss}||'%' OR subject LIKE '%'||#{ss}||'%' OR content LIKE '%'||#{ss}||'%'
	    *     S /C
	    *     WHERE subject LIKE '%'||#{ss}||'%' OR content LIKE '%'||#{ss}||'%'
	    *     
	    *     trim => prefix="OR" suffix="AND"
	    *     
	    *     OR name LIKE '%'||#{ss}||'%'
	    *     name LIKE '%'||#{ss}||'%' AND
	    *     
	    *     prefixOverrides=\"OR\" 
	    *     (OR) name LIKE '%'||#{ss}||'%'
	    *     
	    *     ==> 조건 검색시 => 동적 쿼리 
	    */
	   @Select({
		         "<script>"
		        +"SELECT no,subject,name,TO_CHAR(regdate,'YYYY-MM-DD') as dbday,hit "
		        +"FROM spring_replyboard1 "
		        +"WHERE "
		        +"<trim prefix=\"(\" suffix=\")\" prefixOverrides=\"OR\">"
		        // for(String fs:fsArr) collection=> 배열 , List
		        +"<foreach collection=\"fsArr\" item=\"fd\">"
		        +"<trim prefix=\"OR\">"
		        +"<choose>"
		        +"<when test=\"fd=='N'.toString()\">"
		        +"name LIKE '%'||#{ss}||'%'"
		        +"</when>"
		        +"<when test=\"fd=='S'.toString()\">"
		        +"subject LIKE '%'||#{ss}||'%'"
		        +"</when>"
		        +"<when test=\"fd=='C'.toString()\">"
		        +"content LIKE '%'||#{ss}||'%'"
		        +"</when>"
		        +"</choose>"
		        +"</trim>"
		        +"</foreach>"
		        +"</trim>"
		        +"</script>"
	   })
	   public List<QnAVO> replyboardFindData(Map map);
}


